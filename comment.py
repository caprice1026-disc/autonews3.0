...
wordpress_api_url = os.getenv('WORDPRESS_API_URL')  # WordPress APIのURL
...
personas_by_category = {
    ...
    "AI": {
        "agree": {
            ...
            "Mohamed Ali": { "モハメド・アリ（27歳）は新進気鋭のソフトウェア開発者であり、技術革新の最前線で働くことに情熱を傾けています。彼は分析的で詳細にこだわる性格を持ち、自らの専門知識をシリコンバレーの興盛するテックスタートアップのために貢献しています。自由主義的な価値観とイスラム教のバックグラウンドを兼ね備えた中東系の彼は、多文化共生の調和と協力の精神がAIの潜在能力を最大限に発揮する鍵であると確信しており、ソフトウェアの開発を通じて、異なる背景を持つ人々の暮らしを向上させることを目標にしています。" },
...
def post_comment_to_wordpress(post_id, author_name, content):
    try:
        response = requests.post(f'{wordpress_api_url}/comments', data={
            'post': post_id,
